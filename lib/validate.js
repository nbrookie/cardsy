...
var number = function number(_number) {
  _number = _stringUtils["default"].stripWhiteSpace(_number);

  var template = _cardUtils["default"].getTemplate.byNumber(_number); // Did we find a matching template?
  ...
  if (template.luhn && !_cardUtils["default"].luhnCheck(_number)) {
    return false;
  } // OK!
...
var expiry = function expiry(month, year) {
  ...
  year = year.toString(); // Are month and year numeric strings?

  if (!_stringUtils["default"].isNumeric(month) || !_stringUtils["default"].isNumeric(year)) {
    return false;
  } // Is it a valid month?
  //
  ...
  expiry.setMonth(expiry.getMonth(), 1); //Is the given date in the past?
...
var expiryString = function expiryString(_expiryString) {
  ...
  var parts = _expiryString.split(separator); // Did the string contain three parts with
...
var cvc = function cvc(_cvc, type) {
  _cvc = _stringUtils["default"].stripWhiteSpace(_cvc);

  var template = _cardUtils["default"].getTemplate.byType(type); //Are our parameters present and correct?


  if (!_cvc || !_stringUtils["default"].isNumeric(_cvc) || !template) {
    return false;

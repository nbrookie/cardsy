...
var getTemplateByNumber = function getTemplateByNumber(number) {
  ...
  number = _stringUtils["default"].stripWhiteSpace(number);
  return _templates["default"].find(function (template) {
...
var getAutoCompleteMonth = function getAutoCompleteMonth(month) {
  month = _stringUtils["default"].stripNonNumeric(month);

  if (parseInt(month[0]) > 1) {
    month = "0" + month;
  }

  month = _stringUtils["default"].trimToLength(month, 2);
  return month;
};
/**
 * luhnCheck (Private)

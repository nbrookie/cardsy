...
var _cardUtils = _interopRequireDefault(require("./utils/cardUtils"));
...
var number = function number(_number) {
  var template, maxLength;
  _number = _stringUtils["default"].stripNonNumeric(_number);
  template = _cardUtils["default"].getTemplate.byNumber(_number);
  ...
  var _template = template,
      lengths = _template.lengths,
      spaces = _template.spaces;
  maxLength = lengths[lengths.length - 1] + spaces.length;
  _number = _stringUtils["default"].insertSpaces(_number, spaces);
...
var cvc = function cvc(_cvc) {
  _cvc = _stringUtils["default"].stripNonNumeric(_cvc);
  _cvc = _stringUtils["default"].trimToLength(_cvc, 4);
  return _cvc;
};
/**
...
var expiry = function expiry(month, year) {
  ...
  month = _cardUtils["default"].getAutoCompleteMonth(month);
  year = _stringUtils["default"].stripNonNumeric(year);
...
var expiryString = function expiryString(expiry) {
  ...
  var formattedExpiry, month; //Start by getting the month

  formattedExpiry = month = _cardUtils["default"].getAutoCompleteMonth(_stringUtils["default"].trimToLength(expiry, 2)); //If the month is complete add separator
  ...
  if (_stringUtils["default"].contains(expiry, separator)) {
    var yearIndex = expiry.indexOf(separator) + separator.length;
    var year = expiry.substr(yearIndex, expiry.length - 1);
    year = _stringUtils["default"].stripNonNumeric(year);
    year = _stringUtils["default"].trimToLength(year, 2);
    return formattedExpiry + year;
  }

  if (_stringUtils["default"].containsPartOf(expiry, separator)) {
    return _stringUtils["default"].trimToLength(month, 1);
  }
